...
def check(ip, port=445):
    ...
    try:
        ...
        if nt_status == '\x05\x02\x00\xc0':
            log.info("+++ALERT!!!+++ [+] [{}] is likely VULNERABLE to MS17-010! ({})".format(ip, native_os))
            print("+++ALERT!!!+++ [+] [{}] is likely VULNERABLE to MS17-010! ({})".format(ip, native_os))
            status = 100
...
if __name__ == '__main__':
    ...
    IPs=[]
    for addr in addrs:
          if ':' not in addr[4][0]: #Only IP v4, not IP v6
                IPs.append(addr[4][0])
    ...
    log.info("\n\nCredits: Written by nixawk. Packed and adapted by @joseccnet\n")
